...
- [1901.02031](https://arxiv.org/abs/1901.02031) for the flavor-changing and diphoton rates of the $SU(2)_L$ ALP. Its production includes rare $B$ decays, $K^\pm\to\pi^\pm a$ below threshold, and Primakoff production by primary and electromagnetic-cascade photons. The present decay model uses $\operatorname{Br}(a\to\gamma\gamma)=1$.
- `ALP-mixed` coherently combines the direct photon and $SU(2)_L$ operators.
  The physical lifetime fixes the total diphoton coupling, while the chosen
  operator fraction and interference sign fix the accompanying $B+K$
  production. It reuses the installed pure-model tables; see `README.md` for
  the convention and launch-card example.
...
- Light-meson fluxes (`Pi0`, `Eta`, `Eta'`, `Rho0`, `Omega`, etc.) use the
  Pythia 8 setup of [1904.02091](https://arxiv.org/abs/1904.02091). The present
  flux omits secondary production in the target cascade. These secondaries are
  expected to be soft and largely outside the SHiP acceptance; their numerical
...
The generator-level yield includes the production and decay-volume geometry.
For exported events, `events_analysis.py` provides an additional
model-independent truth-level selection. It propagates visible final-state
particles along straight trajectories to a rectangular detector plane. The
defaults are a $4\,\mathrm{m}\times6\,\mathrm{m}$ plane at $z=95\,\mathrm{m}$.
The user may require exactly two photons, a pointing subset of at least two
particles with zero total charge, or all non-neutrino visible particles to
point to the plane. A common particle-energy threshold is optional; without
one, the cut is simply $E\geq m$. The resulting acceptance is weighted by the
parent decay probability and remains a truth-level geometric quantity, not a
detector reconstruction or efficiency model.
