...
from analysis2.cache import CacheStore, cache_key, canonical_json, file_fingerprint
from analysis2.paths import REPOSITORY_ROOT, portable_path, profile_cache_dir
...
class CacheTests(unittest.TestCase):
    ...
    def test_repository_paths_are_serialized_portably(self):
        path = REPOSITORY_ROOT / "analysis2" / "config.py"
        serialized = canonical_json({"input": path})
        fingerprint = file_fingerprint(path)

        self.assertIn('"input":"analysis2/config.py"', serialized)
        self.assertEqual(fingerprint["path"], "analysis2/config.py")
