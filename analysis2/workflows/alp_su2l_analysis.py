...
from analysis2.conditional_features import FEATURE_LABELS, FEATURE_SUBSETS
...
def run_selected_10k(
    *,
    ...
    command = [
        sys.executable,
        "-m",
        "analysis2.workflows.conditional_feature_selected",
        "--full-domain-dir",
    ...
    *,
    ...
    if not truth_file.is_file():
        subprocess.run(
            [
                sys.executable,
                "-m",
                "analysis2.workflows.conditional_feature_empirical_truths",
                "--selected-dir",
                str(selected_dir),
                "--bank-path",
                str(bank_path),
    ...
    subprocess.run(
        [
            sys.executable,
            "-m",
            "analysis2.workflows.conditional_feature_empirical_validation",
            "--bank-path",
            str(bank_path),
            "--moments-path",
            str(
...
def run_production_subprocess(
    *,
    ...
    command = [
        sys.executable,
        "-m",
        "analysis2.workflows.conditional_feature_production",
        "--bank-path",
        str(bank_path),
        "--domain-path",
        str(domain_path),
