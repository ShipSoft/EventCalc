...
def build_ctau_values(ctau_lower_m: float, ctau_upper_m: float) -> tuple[float, ...]:
    """
    Construct a logarithmic lifetime grid inside the common
    event-rate interval and add one long-lifetime reference.
    """
    lower_m = max(MINIMUM_CTAU_M, float(ctau_lower_m))
    upper_m = float(ctau_upper_m)

    if upper_m < lower_m:
        raise ValueError(f"The common lifetime interval does not extend above {MINIMUM_CTAU_M:g} m.")

    scan_values = np.geomspace(lower_m,upper_m,NUMBER_OF_CTAU_SCAN_POINTS)
...
def distribution_distances(spectrum: dict, reference: dict) -> tuple[float, float]:
    """
    Compare two normalized spectra on their common histogram grid. Returns:
        total_variation_distance: 0.5 * sum_i |p_i - q_i|.
        maximum_cdf_distance: Maximum absolute difference between the two binned CDFs.
    """
...
def plot_model_scan(
    ...
    """
    Plot the normalized energy distributions at all lifetimes for one ALP model.
    """
    ...
    for ctau_m in ctau_values_m:
        ...
        # Do not connect poorly resolved non-empty bins to the statistically reliable part of the curve.
        ...
        # Show low-statistics bins as separate points with Monte Carlo error bars.
    ...
    axis.set_xscale("log")
    axis.set_xlim(mass_gev, ENERGY_MAX_GEV)
...
def make_summary(
    ...
    for (model_name, spectra_by_ctau) in all_spectra.items():
        ...
        for ctau_m in ctau_values_m:
            ...
            # Reuse the statistics already defined in compare_energy_spectra.py.
            summary_row = (
                numerical_summary(
...
def main() -> None:
    ...
    mass_ranges = load_mass_ranges()

    summary_frames = []
    plot_paths = []

    for mass_index, row in enumerate(mass_ranges.itertuples(index=False)):
        mass_gev = float(row.mass_GeV)

        ctau_lower_m = float(row.ctau_lower_m)
        ctau_upper_m = float(row.ctau_upper_m)

        ctau_values_m = build_ctau_values(ctau_lower_m, ctau_upper_m)
        ...
        print(f"Common N_events >= 10 interval: [{ctau_lower_m:.6g}, {ctau_upper_m:.6g}] m")
        ...
        for model_index, (model_name, config) in enumerate(MODEL_CONFIGS.items()):
            ...
            # Same samples at every lifetime for one model and mass, but independent samples between masses.
        ...
        mass_summary["common_ctau_lower_m"] = ctau_lower_m
        mass_summary["common_ctau_upper_m"] = ctau_upper_m
