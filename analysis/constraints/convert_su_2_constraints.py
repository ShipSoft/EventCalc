...
def convert_constraint_file(input_path: Path) -> Path:
    ...
    if data.shape[1] < 2:
        raise ValueError(
            f"{input_path.name} contains fewer than two columns."
        )

    if not np.all(np.isfinite(data[:, :2])):
        raise ValueError(
            f"{input_path.name} contains non-finite masses or couplings."
        )
...
def main() -> None:
    ...
    if not input_paths:
        raise FileNotFoundError(
            f"No bounds_*.txt files found in {RAW_DIR}"
        )

    print(f"alpha_SU2 = {ALPHA_SU2:.12e}")
    print(
        "Coupling conversion factor "
        f"pi/alpha_SU2 = {COUPLING_CONVERSION_FACTOR:.12e}"
    )
    print()
    ...
    for input_path in input_paths:
        ...
        print(
            ...
            f"-> {output_path.relative_to(CONSTRAINTS_DIR)}"
        )
